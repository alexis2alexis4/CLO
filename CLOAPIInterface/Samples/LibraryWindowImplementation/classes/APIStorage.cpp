...
{	
	...
	void APIStorage::GenerateAPIItemListForSample()
	{
		...
		//generateAPIItemListForMyModel();

		//Для test 1
		generateAPIItemListForBeProduct();
	...
	bool APIStorage::GetFilePathWithID(const QString& itemId, QString& resultFilePath)
	{
		...
		if (GetItemWithID(itemId, curItem))
		{
			...
			resultFilePath = SAMPLE_STORAGE_DIRECTORY + GetOnlyFileName(curItem.sampleItemData.itemPath);
			return true;
		}
	...
	bool APIStorage::GetIconThumbnailPathWithID(const QString& itemId, QString& resultFilePath)
	{
		...
		if (GetItemWithID(itemId, curItem))
		{
			if(curItem.sampleItemData.iconThumbnailPath != "")
				...
				resultFilePath = SAMPLE_STORAGE_DIRECTORY + GetOnlyFileName(curItem.sampleItemData.iconThumbnailPath);
			else
				//resultFilePath = SAMPLE_STORAGE_DIRECTORY + curItem.metaData[META_DATA_KEY_0_DATA_TYPE].toString().toLower() + "/" + GetOnlyFileName(curItem.sampleItemData.itemPath);
				resultFilePath = SAMPLE_STORAGE_DIRECTORY + GetOnlyFileName(curItem.sampleItemData.itemPath);
			//UTILITY_API->DisplayMessageBox(resultFilePath.toStdString());
			return true;
		}
	...
	bool APIStorage::GetPreviewThumbnailPathWithID(const QString& itemId, QString& resultFilePath)
	{
		...
		if (GetItemWithID(itemId, curItem))
		{
			if (curItem.sampleItemData.previewThumbnailPath != "")
				...
				resultFilePath = SAMPLE_STORAGE_DIRECTORY + GetOnlyFileName(curItem.sampleItemData.previewThumbnailPath);
			else
				//resultFilePath = SAMPLE_STORAGE_DIRECTORY + curItem.metaData[META_DATA_KEY_0_DATA_TYPE].toString().toLower() + "/" + GetOnlyFileName(curItem.sampleItemData.itemPath);
				resultFilePath = SAMPLE_STORAGE_DIRECTORY + GetOnlyFileName(curItem.sampleItemData.itemPath);
...
//		fabricItem1->dateTime = "2019-01-20T11:11:21";
	...
	void APIStorage::generateAPIItemListForMyModel()
	{
		//Get my model
		LibraryAPIItem* myModelItem1 = newLibraryAPIItemWithRandomizedDate();
		myModelItem1->itemID = "myModel1";
		myModelItem1->itemName = "Dima.obj";
		myModelItem1->metaData[META_DATA_KEY_0_DATA_TYPE] = ITEM_ID_MY_MODEL;
		myModelItem1->metaData["size"] = QString("M");
		myModelItem1->metaData["use"] = QString("obj");
		myModelItem1->sampleItemData.itemPath = "C:/sample_assets/test_dir/r_000_09t.obj";
		myModelItem1->sampleItemData.iconThumbnailPath = "C:/sample_assets/test_dir/texture0.png";
		myModelItem1->sampleItemData.previewThumbnailPath = "C:/sample_assets/test_dir/texture0.png";
		m_LibraryAPIItemList.push_back(myModelItem1);
...
// item1
	...
	//Добавление файла с BeProduct
	void APIStorage::generateAPIItemListForBeProduct()
	{
		...
		if (materials != 0)
		{
			...
			for (int i = 0; i < materials->count; i++)
			{
				if (a->isActive == 1)
				{
					LibraryAPIItem* fabricItem = newLibraryAPIItemWithRandomizedDate();
					fabricItem->itemID = a->id;
					fabricItem->itemName = a->name + "." + a->fileType;
					fabricItem->metaData[META_DATA_KEY_0_DATA_TYPE] = ITEM_ID_FABRIC;
					//fabricItem->metaData[META_DATA_KEY_1_SERVICE_DIVISION] = SD_CLO;
					//fabricItem->metaData[META_DATA_KEY_2_BRAND] = BR_CLO_LUXURY;
					//fabricItem->metaData[META_DATA_KEY_3_PRODUCT_TYPE] = PT_FABRIC_MATTE;
					//fabricItem->metaData[META_DATA_KEY_4_SEASON_CREATED] = SEASON_SPRING_SUMMER_20;

					//fabricItem->metaData["BeProductId"] = a->id;

					fabricItem->sampleItemData.itemPath				= SAMPLE_STORAGE_DIRECTORY + a->name + "." + a->fileType;
					fabricItem->sampleItemData.iconThumbnailPath		= SAMPLE_STORAGE_DIRECTORY + a->name + "." + a->fileType; // thumbnail inside zfab
					fabricItem->sampleItemData.previewThumbnailPath	= SAMPLE_STORAGE_DIRECTORY + a->name + "." + a->fileType; // thumbnail inside zfab
					//QMessageBox::information(NULL, "test", SAMPLE_STORAGE_DIRECTORY + "prev.jpg");
					//fabricItem->sampleItemData.previewThumbnailPath = SAMPLE_STORAGE_DIRECTORY + "prev.jpg"; // thumbnail inside zfab
					m_LibraryAPIItemList.push_back(fabricItem);
				}
